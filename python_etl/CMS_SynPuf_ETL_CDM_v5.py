...
# -----------------------------------
# format datetime in YYYYMMDD
# return in ISO 8601 format - YYYY-MM-DDThh:mm:ss-hh:ss
# setting the timezone to be Eastern time so the time part will be 00:00:00-05:00
# -----------------------------------
def get_datetime_format(date_YYYYMMDD):
    if len(date_YYYYMMDD) == 0:
        return ''
    return '{0}-{1}-{2}T00:00:00-05:00'.format(date_YYYYMMDD[0:4], date_YYYYMMDD[4:6], date_YYYYMMDD[6:8])
...
def write_person_record(beneficiary):
    ...
    person_fd.write('{0},'.format(get_datetime_format(yd.BENE_BIRTH_DT)))                                                                     # birth_datetime
...
def write_death_records(death_fd, beneficiary, death_type_concept_id, cause_source_concept_id):
    ...
    if yd is not None and yd.BENE_DEATH_DT != '':       # if year data for BENE_DEATH_DT is not available, don't write to death file.
        ...
        death_fd.write('{0},'.format(get_datetime_format(yd.BENE_DEATH_DT)))    # death_datetime
...
def write_drug_exposure(drug_exp_fd, person_id, drug_concept_id, start_date, drug_type_concept_id,
        ...
        drug_exp_fd.write('{0},'.format(get_datetime_format(start_date)))                                                          # drug_exposure_start_datetime
        drug_exp_fd.write(',')                                                          # drug_exposure_end_date
        drug_exp_fd.write(',')                                                          # drug_exposure_end_datetime
...
def write_device_exposure(device_fd, person_id, device_concept_id, start_date, end_date, device_type_concept_id,
    ...
    device_fd.write('{0},'.format(get_datetime_format(start_date)))        # device_exposure_start_datetime
    device_fd.write('{0},'.format(get_date_YYYY_MM_DD(end_date)))
    device_fd.write('{0},'.format(get_datetime_format(end_date)))        # device_exposure_end_datetime
...
def write_condition_occurrence(cond_occur_fd, person_id, condition_concept_id,
    ...
    cond_occur_fd.write('{0},'.format(get_datetime_format(from_date)))  # condition_start_datetime
    cond_occur_fd.write('{0},'.format(get_date_YYYY_MM_DD(thru_date)))
    cond_occur_fd.write('{0},'.format(get_datetime_format(thru_date)))  # condition_end_datetime
...
def write_visit_occurrence(visit_occur_fd, person_id, visit_concept_id, visit_occurrence_id, care_site_id, visit_source_concept_id,
    ...
    visit_occur_fd.write('{0},'.format(get_datetime_format(from_date)))  # visit_start_datetime
    visit_occur_fd.write('{0},'.format(get_date_YYYY_MM_DD(thru_date)))
    visit_occur_fd.write('{0},'.format(get_datetime_format(thru_date)))  # visit_end_datetime
...
def write_procedure_occurrence(proc_occur_fd, person_id, procedure_concept_id,
    ...
    proc_occur_fd.write('{0},'.format(get_datetime_format(from_date))) # procedure_datetime
...
def write_measurement(measurement_fd, person_id, measurement_concept_id,
    ...
    measurement_fd.write('{0},'.format(get_datetime_format(measurement_date)))   # measurement_datetime
...
def write_observation(observation_fd, person_id, observation_concept_id,provider_id,
    ...
    observation_fd.write('{0},'.format(get_datetime_format(observation_date)))   # observation_datetime
...
def write_header_records():
    headers = {
        ...
        'condition_occurrence':
            ...
            'condition_end_datetime,condition_type_concept_id,stop_reason,provider_id,visit_occurrence_id,condition_source_value,condition_source_concept_id',

        'procedure_occurrence':
            'procedure_occurrence_id,person_id,procedure_concept_id,procedure_date,procedure_datetime,procedure_type_concept_id,modifier_concept_id,'
        ...
        'drug_exposure':
            'drug_exposure_id,person_id,drug_concept_id,drug_exposure_start_date,drug_exposure_start_datetime,drug_exposure_end_date,drug_exposure_end_datetime,'
            'drug_type_concept_id,stop_reason,refills,quantity,days_supply,sig,route_concept_id,effective_drug_dose,dose_unit_concept_id,'
        ...
        'device_exposure':
            'device_exposure_id,person_id,device_concept_id,device_exposure_start_date,device_exposure_start_datetime,device_exposure_end_date,device_exposure_end_datetime,'
        ...
        'measurement_occurrence':
            'measurement_id,person_id,measurement_concept_id,measurement_date,measurement_datetime,measurement_type_concept_id,operator_concept_id,'
